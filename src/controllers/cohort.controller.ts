...
class CohortController {
	...
	public assignCohort = async (req: RequestWithUser, res: Response, next: NextFunction) => {
		try {
			const assignCohort = req.body as AssignCohort;
			const userId = req.user.id as number;
			const cohortResponse = await this.cohortService.assignCohort(assignCohort, userId);
			AppResponseHelper.sendSuccess(res, 'Success', cohortResponse);
		} catch (ex) {
			next(ex);
		}
